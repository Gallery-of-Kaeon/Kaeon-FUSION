package kaeon_fusion.super_mode;
...
public class SuperMode {
	...
	private static void not(ONEPlusElement element) {
		...
		if(index != -1) {
			...
			element.addElement(new ONEPlusElement(content.substring(index + 3).trim()));
	...
	private static void and(ONEPlusElement element) {
		...
		if(index != -1) {
			...
			ONEPlusElement newElement = new ONEPlusElement(content.substring(index + 3).trim());
